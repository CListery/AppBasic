...
publishing {
    publications.configureEach {
        if (this is MavenPublication) {
            ...
            pom {
                name.set(publicName)
                description.set(AppConfig.Publish.desc)
                url.set(AppConfig.Publish.url)
                licenses {
                    license {
                ...
                developers {
                    developer {
                        id.set(AppConfig.Publish.id)
                        name.set(AppConfig.Publish.name)
                        email.set(AppConfig.Publish.email)
                    }
                }
                scm {
                    url.set(AppConfig.Publish.url)
                    connection.set(AppConfig.Publish.scmUrl)
                    developerConnection.set(AppConfig.Publish.scmUrl)
                }
            }
        }
    }
